...
dem_COVID_data = COVID_data[COVID_data['party']=="D"]
rep_COVID_data = COVID_data[COVID_data['party']!="D"]

# selected data from the scraped tweets
tweets = rep_COVID_data['tweet_cleaned'].tolist()
dates = rep_COVID_data['date'].tolist()
party = rep_COVID_data['party'].tolist()
state = rep_COVID_data['state'].tolist()
username = rep_COVID_data['username'].tolist()

#----------------------------------------------------------------------------------------
# shuffle the tweets so that each corpus is distributed in twitter user and date
def shuffle_indices():
    indices = []
    for i in range(len(rep_COVID_data)):
        indices.append(i)
    random.shuffle(indices)
    indices_file = open('rep_indices.txt', 'a', newline='', encoding='utf8')
    csvWriter = csv.writer(indices_file)
    csvWriter.writerow(indices)
    return indices

indices = shuffle_indices()

# record all the tweets in a txt file
def tweets_saver(indices):
    count = 0
    file = open("rep_tweets.txt","w")
    for i in indices:
...
def tokenizer(type,indices):
    count = 0
    file = open("rep_{}.txt".format(type),"w")
...
def root_finder(type,root_type):
    file = open('rep_{}.txt'.format(type))
    lines = file.readlines()
    file.close()

    output_file = open("rep_{}_{}.txt".format(type,root_type),"w")
...
indices = shuffle_indices()
# save all the tweets before tokenizing
tweets_saver(indices)
