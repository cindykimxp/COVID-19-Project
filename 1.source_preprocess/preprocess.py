...
tweets = COVID_data['tweet_cleaned'].tolist()
dates = COVID_data['date'].tolist()
party = COVID_data['party'].tolist()
state = COVID_data['state'].tolist()
username = COVID_data['username'].tolist()
...
def shuffle_indices():
    ...
    indices_file = open('indices.txt', 'a', newline='', encoding='utf8')
...
def root_finder(type,root_type):
    file = open('{}.txt'.format(type))
    lines = file.readlines()
    file.close()

    output_file = open("{}_{}.txt".format(type,root_type),"w")
